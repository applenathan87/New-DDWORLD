...
2. **한 일은 자동**: 오늘 완료로 체크한 할 일 + 오늘 git 커밋 요약(폴더별 묶음). ~~볼트 리포 포함 여부~~ → 2026-09-12 볼트를 게임 리포(C:\DungeonHR)에 통합했으므로 리포 하나만 보면 된다. 기획 노트 수정도 같은 커밋 요약에 자연히 포함.
...
6. **세션시작·세션종료 스킬 삭제** (게임 리포 `.claude/commands/` 한 벌 — 옛 볼트 쪽은 2026-09-12 볼트 폐기로 함께 사라짐). `.claude/session_start.txt`도 제거. 하던 일은 출근.bat(pull)·출근부(시간·데브로그·커밋)·훅(브리핑)이 대신한다.
