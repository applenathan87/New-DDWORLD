...
**"간파가 재밌고, 도장이 손맛 있는 면접 데스크"** = MVP. 그 MVP를 만드는 과정을, 유니티를 처음 켜는 것부터 하나하나 이해하며 진행한다.
...
- **모델링**: 캐릭터·몬스터 = 복셀(MagicaVoxel) / 환경·책상 소품 = 로우폴리 3D → 유니티. 리액션 애니 2종(긴장/안도) 수준이라 **Blender 풀 리깅은 필요해질 때만** (전투 트랙 대비 애니 부담 대폭↓)
