...
public class BattleTile : MonoBehaviour
{
    ...
    {
        // 몬스터 커스텀 색 우선 (EncounterLibrary 레지스트리)
        if (EncounterLibrary.TryGetColor(data, out var custom))
            return custom;
    ...
    {
        // 몬스터 커스텀 색 우선 (타일은 반투명)
        if (EncounterLibrary.TryGetColor(data, out var custom))
            return new Color(custom.r, custom.g, custom.b, 0.8f);
