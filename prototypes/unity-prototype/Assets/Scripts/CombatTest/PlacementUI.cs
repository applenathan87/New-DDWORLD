...
public class PlacementUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            roundText.text = BuildRoundLabel();
            // 늦게 진입한 경우(이미 손패 보유) 폴백 경로
            // 로스터 모드: 첫 전투만 드로우 연출, 이후는 재정렬 연출 (전량 carryover)
            ...
            StartCoroutine(DrawPhaseSequence3D(playerCarryover));
        }
    }
    ...
    private void OnPhaseChanged(GameManager.GamePhase phase)
    {
        switch (phase)
        {
            case GameManager.GamePhase.Draw:
                // 인카운터 타이틀은 DrawPhaseSequence3D 안에서 표시됨 (중복 제거)
    ...
    {
        // 로스터 모드: 첫 전투만 드로우 연출, 이후는 손패 재정렬 연출 (전량 carryover)
        var gm = GameManager.Instance;
        int playerCarryover = gm.CurrentRound <= 1 ? 0 : gm.PlayerDeck.HandCount;
        StartCoroutine(DrawPhaseSequence3D(playerCarryover));
    }

    /// <summary>인카운터 진행 라벨 (구 라운드 표시 대체)</summary>
    private string BuildRoundLabel()
    {
        var gm = GameManager.Instance;
        var enc = EncounterLibrary.Encounters[gm.EncounterIndex];
        return $"{enc.name}  ({gm.EncounterIndex + 1}/{EncounterLibrary.Count}) | 승 {gm.PlayerWins} · 패 {gm.EnemyWins}";
    }

    // === 3D 드로우 페이즈 (PvE: 플레이어 로스터만) ===

    private IEnumerator DrawPhaseSequence3D(int playerCarryover)
    {
        ...
        var gm = GameManager.Instance;
        var enc = EncounterLibrary.Encounters[gm.EncounterIndex];
        ShowPhaseTitle(enc.name);
        yield return new WaitForSeconds(1.2f);
        ShowFloatingMessage(enc.desc);
        yield return new WaitForSeconds(0.8f);

        Hand3D.Instance.CreatePlayerCards(gm.PlayerDeck.Hand);

        yield return StartCoroutine(Hand3D.Instance.AnimateDrawSequence(playerCarryover, 0));
    ...
    public void UpdateInfoText()
    {
        ...
        infoText.text = $"적 진형을 보고 부대를 배치하세요 ({placed}/5 · 최소 1)";
        roundText.text = BuildRoundLabel();
    }
