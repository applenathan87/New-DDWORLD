...
public class BattleField : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // PvE: GameManager가 먼저 시작해 적 진형이 이미 있으면 표시 (Start 실행 순서 보정)
        if (GameManager.Instance.EnemyPlacements.Count > 0)
        {
            ShowEnemyPlacements();
            EnableFreeCam(); // 배치 중에도 줌/팬 허용
        }
    }
    ...
    {
        if (battleTiles == null) return; // 그리드 생성 전 호출 방지 (Start 순서 보정은 Start에서)
    ...
    private void OnPhaseChanged(GameManager.GamePhase phase)
    {
        switch (phase)
        {
            ...
            case GameManager.GamePhase.Placement:
                // PvE: 적 진형을 보며 카운터 배치 — 전체 필드 뷰 + 자유 카메라(줌/팬)
            ...
            case GameManager.GamePhase.Battle:
                // 적 진형은 이미 배치 페이즈부터 표시됨 (GameManager.StartNewRound)
                freeCamEnabled = false;
                ZoomToBattleView();
