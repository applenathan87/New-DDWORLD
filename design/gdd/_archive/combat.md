...
8. **3판 2선승제** — 먼저 2라운드를 이긴 쪽이 매치 승리 (한 매치 ≈ 6분). 런 모드의 PvP 노드 단위가 된다.
...
| Match End | 2선승 달성 | 최종 정산 완료 | 누적 판돈 정산, 매치 종료. 런 모드의 노드 진행으로 복귀 |
...
| 상대 이탈 (disconnect) | 비동기 PvP에서는 발생하지 않음 (고스트 데이터는 이미 완결됨). 만약 고스트 풀이 없으면 AI 대전으로 대체 | 비동기 구조에서 매치 완결 자동 보장. [ADR-001](../../docs/architecture/ADR-001-async-pvp.md) 참조 |
...
| 선승 조건 | 2선승 (3판) | 2선승 ~ 3선승 | 긴 매치, 역전 기회 증가 | 빠른 매치 |
