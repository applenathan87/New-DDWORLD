...
| Deck | MVP 범위 밖 | 판돈 → 덱 강화는 확장 옵션. 프로토타입에서 판돈은 점수 역할만 |
...
| 판돈 외 메타 자원이 있는가? | nathan | 해결됨 | 프로토타입에서 판돈은 점수 역할만. 메타 자원은 MVP 범위 밖 |
| 패자에게도 소량 보상? | nathan | [미정] | 연패 시 이탈 방지 고려. 프로토타입 후 결정 |
| 시즌/랭크 시스템과의 연결? | nathan | MVP 범위 밖 | 향후 확장 |
| 덱 강화로 뭘 살 수 있는가? | nathan | MVP 범위 밖 | 프로토타입에서는 판돈=점수. 덱 강화 상점은 프로토타입 이후 설계 |
